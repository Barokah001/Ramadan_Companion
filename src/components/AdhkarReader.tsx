...
// src/components/AdhkarReader.tsx - FIXED: morning/evening fully independent progress
...
export const AdhkarReader: React.FC<AdhkarReaderProps> = ({
  ...
  const [progress, setProgress] = useState<DhikrProgress>(EMPTY_PROGRESS);
  // Track whether the current progress has been loaded (to avoid premature saves)
  const loadedRef = useRef(false);

  const currentAdhkar = activeTab === "morning" ? morningAdhkar : eveningAdhkar;

  // ── Load progress for the active tab ──
  // Each tab has its own isolated storage key: adhkar-morning-YYYY-MM-DD
  useEffect(() => {
    loadedRef.current = false; // mark as not-yet-loaded while we fetch
    setProgress(EMPTY_PROGRESS); // clear UI immediately on tab switch
    ...
    const loadProgress = async () => {
      try {
        ...
        loadedRef.current = true; // now safe to save
      }
    };
  ...
  // ── Save progress (only after it has been loaded) ──
  useEffect(() => {
    if (!loadedRef.current) return; // don't save the empty reset state
    if (
  ...
  const resetAll = () => {
    ...
    const key = `adhkar-${activeTab}-${today}`;
    storage.set(key, JSON.stringify(EMPTY_PROGRESS)).catch(console.error);
  };
